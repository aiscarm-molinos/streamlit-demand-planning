...
- **Histórico de ventas**: KPI "Promedio % Variación Mensual" (ruidoso) reemplazado por YoY del último mes cerrado; alerta `st.error` si la variación mes a mes cae ≤-15%; Mix Productos de pie a barra horizontal (más legible con 8 categorías).
- **Forecast**: línea "Mes en curso" en el área (ver bug de `add_vline` arriba).
- **Plan Anual**: KPI "Proyección run-rate" (ritmo de meses YA cerrados del año × 12, independiente del Forecast Consensuado oficial que ya está adentro de "Histórico + FCST") + badge 🟢/🟡/🔴 de brecha (antes solo el delta binario de `st.metric`).
