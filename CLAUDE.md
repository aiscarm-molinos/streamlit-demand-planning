...
- **El banner "Gran División: X" ya está** arriba del gráfico principal en Forecast y Propuestas FCST (`st.caption(f"Gran División: {division_actual}")`, con `division_actual = filtros.get("ZBIGDIVISION", [None])[0]`) -- Histórico de ventas y Plan Anual no lo tienen (no se confirmó ese banner en sus capturas originales antes de borrarlas).
